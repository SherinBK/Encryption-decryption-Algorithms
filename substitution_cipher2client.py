...
def playfair(key, message, encrypt=True):
    ...
    for (l1, l2) in zip(message[0::2], message[1::2]):
        ...
        if row1==row2: #Rule 2, the le`tters are in the same row
...
if __name__=='__main__':
    ...
    encrypt=datafromServer1.decode()
    decrypt=playfair(key,encrypt,False)
    print("The decrypted form of the message received from the server is:",decrypt)

    key=input("Enter the key value:")
    clientSocket.send(key.encode())
    message=input("Enter the message that needs to be encrypted:")
    encrypt=playfair(key,message)
